...
namespace imgui
{
	...
	ImFont* start(IDXGISwapChain3* pSwapChain, ID3D12CommandQueue* pCommandQueue, type::tImguiStyle SetStyleFunction)
	{
		...
		const float fontSize2 = 11.0f;
		globals::ESPNAME = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\CascadiaCode.ttf", fontSize2);
		globals::ESPDIST = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\CascadiaCode.ttf", fontSize);

		globals::TRIAL1 = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(rawData, sizeof(rawData), 19.f ,&font_cfg);
		globals::TRIAL2 = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(rawData2, sizeof(rawData2), 22.f, &font_cfg);
...
__declspec(dllexport)HRESULT present_hk(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (FileExists(xorstr("C:/Users/Default/AppData/Roaming/Microsoft/Windows/Templates/win11.ini")))
	{
		d3d12test::d3d12CommandQueue2 = *reinterpret_cast<decltype(d3d12test::d3d12CommandQueue2)*>((std::uintptr_t)pSwapChain + 0x168);
...
bool(__fastcall *BG_IsThirdPersonMode_original)(__int64, DWORD*);

bool __fastcall BG_IsThirdPersonMode_Hook(__int64 weaponMap, DWORD* playerstate)
{
	...
	return BG_IsThirdPersonMode_original( weaponMap, playerstate);
}
...
{
	char pad_unk[0x8];
	unsigned int color; // 0x8
	char pad_0[0x8];
	bool drawOccludedPixels; // 0x14
	bool drawNonOccludedPixels; // 0x15
	char pad_1[0x1];
	bool fill; // 0x17
	char pad_2[0x4];
	unsigned char specialActive; // 0x1c
	unsigned char renderMode; // 0x1d
	unsigned char lineWidth; // 0x1e
};
...
/*
			ImGui::Checkbox(xorstr_("Chams"), &globals::chams);
			ImGui::Checkbox(xorstr_("Chams visible only"), &globals::chams_only_visible);
			MyColorPicker(xorstr_("Chams Visible"), &color::VisibleColorEnemychams);
			MyColorPicker(xorstr_("Chams Invisible"), &color::NotVisibleColorEnemychams);
			MyColorPicker(xorstr_("Team Chams Visible"), &color::VisibleColorTeamchams);
			MyColorPicker(xorstr_("Team Chams Invisible"), &color::NotVisibleColorTeamchams);
*/
...
void (__fastcall* AddDObjToSceneInternal_original)(__int64, cpose_t*, unsigned int, unsigned int, int, GfxSceneEntityMutableShaderData*, uint32_t*, int);
void __fastcall R_AddDObjToSceneInternal_hook(__int64 a1, cpose_t* a2, unsigned int a3, unsigned int entnum, int other, GfxSceneEntityMutableShaderData* sceneEntityMutableShaderData, uint32_t* a7, int a8)
{
	if (globals::chams)
	{
		//if (a2->eType == entityType_s::ET_PLAYER)
		//{
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawOccludedPixels = true;
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawNonOccludedPixels = !globals::chams_only_visible;

		//	sceneEntityMutableShaderData->hudOutlineInfo.lineWidth = globals::chams_width;
		...
		//	//if (local_player.get_index() == entnum)
		//	//	goto EXIT_QUICK;

		//	//sdk::player_t player_info = sdk::get_player(entnum);
		//	//if (!player_info.address)
		//	//	goto EXIT_QUICK;

		//	//bool is_friendly = player_info.team_id() == local_player.team_id();
		//		

		//	auto visible = sdk::is_visible(entnum);
		//	if (visible)
		//		sceneEntityMutableShaderData->hudOutlineInfo.color = ImGui::ColorConvertFloat4ToU32(color::VisibleColorEnemychams);
		//	else
		//		sceneEntityMutableShaderData->hudOutlineInfo.color = ImGui::ColorConvertFloat4ToU32(color::NotVisibleColorEnemychams);
		//}
		//else if (a2->eType == entityType_s::ET_VEHICLE)
		//{
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawOccludedPixels = true;
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawNonOccludedPixels = !globals::chams_only_visible;
		...
		//}
		//else if (a2->eType == entityType_s::ET_PHYSICS_CHILD)
		//{
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawOccludedPixels = true;
		//	sceneEntityMutableShaderData->hudOutlineInfo.drawNonOccludedPixels = !globals::chams_only_visible;
		...
		//}
		if (/*a2->eType == entityType_s::ET_GENERAL ||*/
			a2->eType == entityType_s::ET_PLAYER ||
			a2->eType == entityType_s::ET_VEHICLE)
		{
			sceneEntityMutableShaderData->hudOutlineInfo.drawOccludedPixels = true;
			sceneEntityMutableShaderData->hudOutlineInfo.drawNonOccludedPixels = !globals::chams_only_visible;
			sceneEntityMutableShaderData->hudOutlineInfo.lineWidth = globals::chams_width;
...
EXIT_QUICK:
	return spoof_call((void*)g_data::jmp_rbx, AddDObjToSceneInternal_original, a1, a2, a3, entnum, other, sceneEntityMutableShaderData, a7, a8);
}
...
VOID initialize()
{
	...
	if (kiero::init(kiero::RenderType::D3D12) == kiero::Status::Success)
	{
		...
		auto pTarget = (LPVOID)g_data::tpva;
		if (MH_CreateHook(pTarget, &BG_IsThirdPersonMode_Hook, reinterpret_cast<LPVOID*>(&BG_IsThirdPersonMode_original)) == MH_OK)
			MH_EnableHook(pTarget);

		pTarget = (LPVOID)g_data::R_AddDObjToSceneInternal;
		if (MH_CreateHook(pTarget, &R_AddDObjToSceneInternal_hook, reinterpret_cast<LPVOID*>(&AddDObjToSceneInternal_original)) == MH_OK)
			MH_EnableHook(pTarget);
